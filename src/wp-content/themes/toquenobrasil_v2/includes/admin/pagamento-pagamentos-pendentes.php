...
WHERE 
	...
	$wpdb->posts.ID NOT IN (SELECT post_id FROM $wpdb->postmeta WHERE meta_key = 'inscricao_pagamento_efetuado')
...
?>
	...
	<h3>Eventos com inscrições encerradas e pagamentos ainda não repassados para os produtores</h3>
    <table class='widefat'>
        <thead>
            <tr>
                <th>evento</th>
        ...
        ?>
            <tr>
                ...
                <td style='text-align:right;'>
                	...
                	<a href="javascript:marcarPagamentoComoEfetuado('<?php echo htmlentities(utf8_decode($evento->post_title))?>',<?php echo $evento->ID?>)">marcar como pagamento efetuado</a>
                </td>
            </tr>
        <?php endforeach;?>
        </tbody>
...
<!--
function marcarPagamentoComoEfetuado(evento_nome, evento_id){
	var form = document.getElementById('marcar-pagamento-como-efetuado');
	if(confirm('confirmar o pagamento das inscrições do evento '+evento_nome+' para o produtor?')){
		form.evento_id.value = evento_id;
		form.submit();
	}
