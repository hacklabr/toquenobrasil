...
if(isset($_GET['registros_de']) && trim($_GET['registros_de'])){
	...
	$sql_de = "$wpdb->users.user_registered >= '$registros_de' AND";
}
...
if(isset($_GET['registros_ate']) && trim($_GET['registros_ate'])){
	...
	$sql_ate = "$wpdb->users.user_registered <= '$registros_ate' AND";
}
...
SELECT 
    ...
    (INSTR(wp_usermeta.meta_value,'produtor') > 0) AS is_produtor
FROM
	$wpdb->users,
	$wpdb->usermeta
WHERE
...
foreach ($users as $user){
	$dia = substr($user->user_registered, 0, 10);
	$mes = substr($user->user_registered, 0, 7);
...
<div id='registros-por-dia' class='graph' style='width:1000px; height:250px;'></div>

<h4>acumulado no período</h4>
<div id='acumulado-por-dia' class='graph' style='width:1000px; height:250px;'></div>
...
var data_dia = [
                	{data: artistas_data, label: 'artistas'},
                	{data: produtores_data, label:'produtores'}
                ];
