...
//$sql_select_artistas = "SELECT * FROM $wpdb->users WHERE ID > 300 AND ID < 400";
...
foreach ($artistas as $artista){
    ...
    $post_imagens = wp_insert_post($imagens);
    wp_publish_post($post_imagens);
    echo $post_imagens ? '(OK)' : '<strong>(ERROR)</strong>';
    ...
    // VIDEOS
    $video = get_user_meta($artista->ID, 'youtube', true);
    
    if ($video) {
    
        $postVideo = array(
            'post_title' => $artista->display_name,
            'post_status' => 'publish',
            'post_content' => '',
            'post_author' => $artista->ID,
            'post_excerpt' => $video,
        ...
        $postVideo = wp_insert_post($postVideo);
